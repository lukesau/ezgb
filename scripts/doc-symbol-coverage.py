#!/usr/bin/env python3
"""Rank what to label next from docs / kernel.sym / call fan-in / orphans.
...
# An absolute WRAM / FPGA touch means not a pure compiler leaf.
...
def is_padding_or_trap(body):
    """Bank fillers / RST $38 traps are not naming targets."""
...
def insn_size(line):
    ...
    if re.match(r"(jp|call)\s+", s):
        return 3
    # ld a, [wLabel] and ld [wLabel], a are absolute 16-bit (FA/EA).
...
def rank_candidates(
    ...
    # Per-bank sorted named ROM *code* addrs. A Jump_ strictly between two named
    ...
    def is_interior_jump(bank, addr_s):
        ...
        for na in addrs:
            if na > lo:
                return addr < na
        # No named symbol after lo, so still interior to that last named function
