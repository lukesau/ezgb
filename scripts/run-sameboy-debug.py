...
def ensure_kernel_sym(version):
    ...
    source of truth for mgbdis, not a disposable build artifact, so it must
...
def main():
    ...
    if args.trace:
        ...
        print("Navigate the browser and press A on a .GB; the dump stops automatically.")
    else:
        if args.script:
            ...
            if fd in r:
                ...
                if args.trace:
                    ...
                    if STOP_RE.search(text) and PROMPT_RE.search(text):
                        ...
                        # $7fe0=$80 replaces the kernel ROM, so stop auto-continue
                        boot = re.search(
