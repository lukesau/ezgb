...
by hand when a feature lands (edit the file, and the next content-changing
...
def make_one(version):
    ...
    if stock == modded:
        sys.exit("error: kernel.gb is identical to kernel.gb.orig, so there is nothing to patch")
