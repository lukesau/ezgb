...
Hand-assembled, not SDCC-compiled, for the same reason as `shims.md`, but
with a different constraint: these are not far-call trampolines, they are
*frame-preserving* entry stubs.
...
So each stub is **jumped to**, never called (SP is therefore still exactly
the `FileBrowserEntry` frame pointer on entry), and it computes the frame
address *before* pushing anything:
