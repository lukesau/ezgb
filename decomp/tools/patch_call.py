...
bytes, the remainder is filled with `nop` (never fewer than 3, since that
would overwrite mid-instruction and corrupt whatever follows).
...
def main():
    ...
    # or FarCallTrampoline, not a bare `call`, and this tool doesn't do that.
