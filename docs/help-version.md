...
the one part that differs between the kernels (`VERSION_TEXT` in
`scripts/port-mod.py`). Bank 8 is byte-identical across all three builds, so
the hook and its call site are the same everywhere.
