...
The menu kernel entry point, after the factory bootstrap has loaded `ezgb.dat`.
SameBoy skips the bootstrap and jumps here directly; see
[`ezgb-dat-boot.md`](ezgb-dat-boot.md) for the bootstrap layer.
...
| `$0150` `Jump_000_0150` | n/a | Real C / runtime start |
...
Foreground boot enters `Call_000_0de4` and stays there for the browser. The `$01DF` halt after `call $1835` is real bytes but a dead exit for the normal boot-to-menu path.
...
- Not a single verify target: `$0150` and `Call_000_0de4` call many unknowns; need
