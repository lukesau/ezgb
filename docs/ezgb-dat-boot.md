...
They are not interchangeable. Skipping `Update_FW*.gb` is fine if the cart's FPGA already reports the FW version you need (HELP tab). Updating the kernel: replace `ezgb.dat` on the SD root and reboot.

Official FW4 package `readme.txt` documents only the updater-ROM path for FPGA. The SD-root `ezgb.dat` path is how a bare cart obtains a kernel at all.
...
Blank SD producing this screen on every cold boot shows the menu is not permanently
installed as the sole OS; the cart expects `ezgb.dat` from disk each boot.
...
Conclusion: once the factory bootstrap has loaded the kernel, the running OS treats `ezgb.dat` as an invisible support file, not something it re-flashes on every boot from this code path.

Those bootstrap strings (`ezgb.dat not found` / `Please download it` / `from www.ezflash.cn`)
are absent from `ezgb.dat` and `Update_FW*.gb`. That UI lives only in the factory layer, which
we do not have a dump of.
...
That path programs FPGA firmware, not the menu image. A cart that already has a matching FW can run indefinitely on SD-only `ezgb.dat` drops.
