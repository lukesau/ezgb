...
streaming. It configures nothing about the game either, assuming the MBC
...
Everything upstream of `$1623` is game *configuration*, not the copy:
`RomLoaderMain` header read/checksum, `LaunchSetup`, and the whole FPGA
config block (`$7fc0`, `$7f37`, `$7fd4`, `$7fc4`, `$7fc1/$7fc2`, `$7fc3`).
...
so a stale flag can't turn a later normal launch into a wrong-game no-copy
boot.
...
**Confirmed in the emulator (SameBoy + EZ Jr stub, 2026-08-30):**
...
- **The game store is a pSRAM die, and it's U9's, not U4's** (chip map, see
  [hardware-board.md](hardware-board.md)): deduced from U4's datasheet, since
  an 8 MB ROM can't fit in U4's 512 KB pSRAM die and must live in a larger
