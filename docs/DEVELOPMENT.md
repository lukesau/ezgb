...
- **[SDCC](https://sdcc.sourceforge.net/)** (4.6.0+ from brew is fine for
  simple functions) — compiles the C for `decomp/` and for injected features.
- **[SameBoy](https://sameboy.github.io)** with the EZ Jr FPGA stub — dynamic
...
`mgbdis` must run with `--overwrite` when `disassembly/` already exists, or
bank files keep their old `Call_` names. `annotate-disasm.py` also emits
`wram.inc` for `kernel.sym` entries at CPU addr ≥ `$C000` (mgbdis rewrites
`[wGfxMode]` but does not define those labels).

The build reassembles **byte-identical** to the original dump (only the 3
cosmetic ROM-header bytes differ — see [REGISTERS.md](REGISTERS.md)).

Persistent annotations live in two files per version:

- [`re/1.05e/kernel.sym`](../re/1.05e/kernel.sym) — names (`Call_000_0de4` →
...
The disassembly is already 100% labeled, but naming/annotation is an ongoing
...
`map-next` prints the body, callers, WRAM/`$7Fxx` touches, and
`needs_judgment` for its top target — do not re-grep what it already showed.
...
function-by-function in C, verified byte-identical against the original
compiled output — the same way the Pokémon Gen 1–3 or Super Mario 64
decompilations work. `re/` is the research phase; `decomp/` is the rewrite.
Secondary to naming the ASM; see [PROGRESS.md](PROGRESS.md).
...
with `sdcc -msm83 --sdcccall 0` produces `1E 00 C9`, byte-for-byte identical to
`Call_000_1a77` in the real 1.05e kernel. See `decomp/src/misc.c` and
...
1. Pick a function from `re/1.05e/disassembly/bank_*.asm` (or `1.04e`). Note
   its bank and address.
...
SDCC emitted for the C file under test; pinned symbols contribute address
fixups only.
...
inject → hook → run chain end to end, documents the free-space map, and
explains how to pick a safe hook site (and the two ways the earlier fastlaunch
attempt silently corrupted the ROM). Read it before choosing an address.
...
- **[SameBoy](https://sameboy.github.io)** — Game Boy emulator with a debugger,
  for dynamic tracing. Upstream stays out of git; our EZ Jr FPGA stub is
...
- **[omega-de-kernel](https://github.com/ez-flash/omega-de-kernel)** — EZ
  Flash's own published Omega DE (GBA) kernel source: FPGA register naming,
  FatFs/UI structure, NOR / Mode B boot path. Pair with a real Omega cart for
  UX side-by-side; see [omega-jr-compare.md](omega-jr-compare.md).
